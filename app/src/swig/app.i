...
static void RegisterLibraryHelper(const char* library, const char* version) {
#if FIREBASE_PLATFORM_ANDROID
    JNIEnv* jni_env;
    jobject activity_local_ref = UnityGetActivity(&jni_env);
    firebase::util::CallAfterEnsureMethodsCached(
            jni_env, activity_local_ref, [&library, &version, &jni_env](){
        firebase::App::RegisterLibrary(library, version, jni_env);
    });
    jni_env->DeleteLocalRef(activity_local_ref);
#else
    firebase::App::RegisterLibrary(library, version, nullptr);
#endif
}
...
%extend firebase::App {
  ...
  static void RegisterLibraryInternal(const char* library, const char* version) {
    firebase::RegisterLibraryHelper(library, version);
  }
