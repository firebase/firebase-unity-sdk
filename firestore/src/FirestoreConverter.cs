...
namespace Firebase.Firestore {
  ...
  /// handling into the Firestore data model representation. Deserialization works the same way: first the Firestore
  /// data model representation is converted into .NET types in the default manner for this library, and the result is
  /// passed to the converter. Note that this means there can be asymmetry between serialization and deserialization: if the
  /// <see cref="M:FirestoreConverter`1{T}.ToFirestore(T)"/> method returns an <see cref="Int32"/>, deserializing the result will involve
  ...
  /// from <see cref="M:FirestoreConverter`1{T}.ToFirestore(T)"/>. On deserialization, <see cref="FromFirestore(object)"/> will be passed a value
  /// implementing <see cref="IDictionary{TKey,TValue}"/> with a key type of <see cref="string"/> and a value type
  /// of <see cref="object"/>.
  /// </para>
  /// </remarks>
  /// <typeparam name="T">The type of object to convert.</typeparam>
  public abstract class FirestoreConverter<T> {

    /// <summary>
    /// Creates a new instance of this class.
    /// </summary>
    public FirestoreConverter() {
      // This is a hack for AOT platforms such that a CustomConverter implementation will be generated for type T.
      // This is also the reason why `FirestoreConverter` is not an interface.
      CustomConverter.CreateInstance<T>(null);
    }

    /// <summary>
    /// Converts a value to its Firestore representation.
    /// </summary>
    /// <param name="value">The value to convert. When called by Firebase.Firestore,
    /// this will never be null.</param>
    /// <returns>The converted value. Must not be null.</returns>
    public abstract object ToFirestore(T value);
    ...
    /// this will never be null.</param>
    /// <returns>The converted value. Must not be null.</returns>
    public abstract T FromFirestore(object value);
  }
}
