...
namespace Firebase.Firestore {
  ...
  public sealed class FirestoreEnumNameConverter<T> : FirestoreConverter<T>
      where T : struct {
    ...
    static FirestoreEnumNameConverter() {
      // HACK: IL2CPP strips this constructor without it.
      new FirestoreEnumNameConverter<T>();

      if (!typeof(T).IsEnum) {
        throw new ArgumentException("Can't use FirestoreEnumNameConverter on {0} since it is not an enum.", typeof(T).FullName);
      }
    ...
    /// <inheritdoc />
    public override T FromFirestore(object value) {
      if (s_nameToValueMap.TryGetValue((string)value, out var result)) {
        return result;
      } else {
        throw new ArgumentException(String.Format("Unknown name '{0}' for enum {1}", value, typeof(T).FullName));
      }
    }
    ...
    public override object ToFirestore(T value) {
      if (s_valueToNameMap.TryGetValue(value, out var result)) {
        return result;
      } else {
        throw new ArgumentException(String.Format("Unknown value '{0}' for enum {1}", value, typeof(T).FullName));
      }
    }
  }
}
