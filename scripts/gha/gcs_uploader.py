...
def main(argv):
  ...
  logging.info("Looking for files ending in '.log'...")
  artifacts = []
  for file_dir, dir_names, file_names in os.walk(testapp_dir):
    if "testapp" in dir_names:
      # We'll most likely want to upload the Unity project as well.
      # To do this, we can zip it with shutil.make_archive and upload that.
      dir_names.remove("testapp")  # Don't look for artifacts in Unity project.
    for file_name in file_names:
      if file_name.endswith(".log"):  # Upload all .log files
