...
flags.DEFINE_list(
    ...
    "Build for device or simulator (only affects iOS and tvOS).")
...
flags.DEFINE_bool(
    ...
    "Tell Unity to symlink iOS/tvOS libraries into the xcode project"
...
def build_testapp(dir_helper, api_config, ios_config, target):
  ...
  For iOS and tvOS, this will generate an xcode project. For other platforms,
  this will generate the final binary (apk or standalone binary).
  ...
  while not test_finished and time_until_timeout > 0:
    ...
    if os.path.exists(log):
      ...
      if retry_on_license_check and "License updated successfully" in text:
        ...
        perform_in_editor_tests(dir_helper, retry_on_license_check=False, remaining_retries=remaining_retries)
        return
  open_process.kill()
  ...
  if results.complete:
    if results.passes and not results.fails:  # Success
      ...
      if remaining_retries > 0:
        ...
        perform_in_editor_tests(dir_helper, retry_on_license_check=retry_on_license_check, remaining_retries - 1)
        return
...
def get_xcode_name(version, force_xcode_project):
  ...
  Returns:
    ...
    workspace used for this build. Note that Unity exports tvOS xcode projects
    using the same name as that used for iOS.
  """
  version = unity_version.UnityVersion(version)
