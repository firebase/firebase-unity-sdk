...
flags.DEFINE_string(
    "build_workflow", 'build_starter.yml',
    "The build workflow in the repo that runs the nightly build.")

flags.DEFINE_string(
    "test_workflow", 'integration_tests.yml',
    "The test workflow in the repo that runs the nightly build.")
...
def main(argv):
  ...
  if FLAGS.read_cache:
    ...
    with progress.bar.Bar('Downloading test summaries...', max=len(package_tests)) as bar:
      for day, run in package_tests.items():
  ...
  if FLAGS.report == "daily_log":
    ...
    for test_id in test_list_sorted:
      ...
      if FLAGS.output_markdown:
        ...
        if m:
          search_url = "http://github.com/search?q=repo:firebase/firebase-unity-sdk%%20\"%s,%%20%s\"" % (m.group(1), m.group(2))
