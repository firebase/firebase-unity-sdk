...
def validate_results_unity(log_text):
  ...
  """
  # The result summary looks like 'PASS: 4, FAIL: 7'
  results_match = re.search(
      r"PASS: (?P<pass>[0-9]+), FAIL: (?P<fail>[0-9]+)", log_text)
  if results_match:
    summary = results_match.group(0)
    # We also want the list of failed test cases, which are listed after
    # the result summary and look like this:
    # TestInstanceIdChangeAfterReset (6/8)
    log_tail = log_text[results_match.start():]
    failed_cases = re.findall(r"Test\w+ \(\d+\/\d+\)", log_tail)
    if failed_cases:
      summary += "\n" + "\n".join(failed_cases)
    ...
    return TestResults(
        complete=True,
        passes=int(match_dict["pass"]),
        fails=int(match_dict["fail"]),
        summary=summary)
  else:
    return TestResults(complete=False, summary=_tail(log_text, 15))
